...
var pendingCharacterId = null;
var container = null;

function requestCreatorNotes(characterId) {
  spindle.sendToBackend({
    type: "fetch_creator_notes",
    character_id: characterId
  });
}

spindle.onBackendMessage(function(msg) {
  if (msg.type === "creator_notes_response") {
    if (msg.error) {
      container.innerHTML = renderPreview('<p style="color:#f87171">Failed to read creator notes: ' + msg.error + "</p>");
    } else {
      container.innerHTML = renderPreview(msg.creator_notes || "<p style='opacity:0.5'>Creator notes are empty.</p>");
    }
  }
});

spindle.registerFloatWidget("html_preview", {
  label: "HTML Preview",
  icon: "code",
  width: 480,
  height: 400,
  ...
  mount: function(el) {
    container = el;
    var characterId = spindle.getActiveCharacterId ? spindle.getActiveCharacterId() : null;
    if (characterId) {
      requestCreatorNotes(characterId);
    } else {
      el.innerHTML = renderPreview("<p style='opacity:0.5'>Open a character in the editor to see a live HTML preview of their creator notes.</p>");
    }
  },
  onActiveCharacterChange: function(characterId) {
    if (characterId) {
      requestCreatorNotes(characterId);
    } else {
      if (container) {
        container.innerHTML = renderPreview("<p style='opacity:0.5'>No character selected.</p>");
      }
    }
  }
});
